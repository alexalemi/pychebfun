...
class Cheb(object):
    ...
    def quad(self):
        ...
        weights = np.fromfunction(lambda x: 2.0 / (1 - (2 * x) ** 2), goodcoeffs.shape)
    ...
    def __repr__(self):
        out = "<{}(N={},domain={},rtol={},naf={})>".format(
            self.__class__.__name__, self.__len__(), self.domain, self.rtol, self.naf
        )
        return out
    ...
    def _eval(self, arg):
        ...
        if isinstance(arg, Cheb):
            ...
            assert (
                mya <= othera and myb >= otherb
            ), "Domain must include range of other function"
            return self._new_func(
                lambda x: self._eval(arg._eval(x)),
                arg.domain,
                rtol=min(arg.rtol, self.rtol),
            )
...
class Chebfun(Cheb):
    ...
    def __init__(self, funcs=None, edges=None, imps=None, chebs=None, rtol=None):
        ...
        self.chebs = chebs or [
            Cheb(func, domain=edgepair, rtol=self.rtol)
            for (func, edgepair) in zip(
                self.funcs, list(zip(self.edges, self.edges[1:]))
            )
        ]
        ...
        if not imps:
            ...
            pairs = list(zip(list(range(nfuncs)), list(range(1, nfuncs))))
            self.imps = (
                [self.funcs[0](self.edges[0])]
                + [
                    0.5 * (self.funcs[i](x) + self.funcs[j](x))
                    for ((i, j), x) in zip(pairs, self.edges)
                ]
                + [self.funcs[-1](self.edges[-1])]
            )
        else:
            self.imps = imps
    ...
    def __add__(self, other):
        ...
        try:
            ...
            newchebs = [
                cheb._new_func(
                    lambda x: cheb._eval(x) + other._eval(x),
    ...
    def __repr__(self):
        out = "<{}(nfuncs={},edges={},rtol={},naf={})>".format(
            self.__class__.__name__, self.__len__(), self.edges, self.rtol, self.naf
        )
        return out
