...
def detectedge(f, a, b, hs=None, vs=None, der=None, checkblowup=True):
    ...
    while (
        (maxd[nder - 1] != inf)
        and (~isnan(maxd[nder - 1]))
        and (diff(ends) > DEFAULT_TOL * hs)
    ):
        maxd1 = maxd[:nder]
        (na, nb, maxd) = maxder(f, ends[0], ends[1], nder, N, der)
        c1 = (4.5 - arange(nder)) * maxd1
        c2 = 10.0 * vs / (hs ** arange(nder))
        ...
        if checkblowup and abs(f((ends[0] + ends[1]) / 2.0)) > 100 * vs:
...
def findjump(f, a, b, hs=None, vs=None, der=None):
    ...
    e1 = (b + a) / 2.0  # estimate edge location
    e0 = e1 + 1  # force loop
